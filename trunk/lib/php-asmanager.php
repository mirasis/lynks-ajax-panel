...
class AGI_AsteriskManager
{
	...
	*/
	...
	function send_request2($action, $parameters=array())
	{
		$req = "Action: $action\r\n";
		foreach($parameters as $var=>$val)
			$req .= "$var: $val\r\n";
		$req .= "\r\n";
		fwrite($this->socket, $req);
                ?><pre>$req<?print_r($req);?></pre><?
		return $this->wait_response2();
	}	
	/**
	* Wait for a response
	*
	...
	*/
	...
	function AgentCallBackLogin($Agent, $Exten, $Context, $AckCall, $WrapupTime, $ActionID)
	{
		$parameters = array();
		if($Agent) $parameters['Agent'] = $Agent;
		if($Exten) $parameters['Exten'] = $Exten;
		if($Context) $parameters['Context'] = $Context;
		if($AckCall) $parameters['AckCall'] = $AckCall;
		if($WrapupTime) $parameters['WrapupTime'] = $WrapupTime;
		if($ActionID) $parameters['ActionID'] = $ActionID;
		return $this->send_request('AgentCallBackLogin', $parameters);
	}
        function AgentLogoff($Agent)
	{
		$parameters = array();
		if($Agent) $parameters['Agent'] = $Agent;
		return $this->send_request('AgentLogoff', $parameters);
	}
        function AgentStatus()
	{
		return $this->send_request('Agents');
	}
	function QueueAdd($Queue, $Interface, $Penalty, $Paused)
	{
		$parameters = array();
	...
	function QueuePause($Queue, $Interface, $Paused)
	{
		$parameters = array();
		if($Queue) $parameters['Queue'] = $Queue;
		if($Interface) $parameters['Interface'] = $Interface;
	...
	function QueueRemove($Queue, $Interface)
	{
		...
		if($Queue) $parameters['Queue'] = $Queue;
		if($Interface) $parameters['Interface'] = $Interface;
		return $this->send_request('QueueRemove', $parameters);
	}
}
?>
